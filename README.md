...
- 背景音樂使用提供的《宇宙奧祕》M4A（AAC），存於 `assets/audio/cosmic-mystery.m4a`，循環播放。隨速度由原速漸增至接近 1.8 倍，盡量保留音高；此音樂保護不限制航行速度。暫停／收藏卡畫面暫停播放，繼續時接續原位置，重新開始從歌曲開頭播放。
...
MP3 變速最多每秒更新一次，忽略微小變化，避免頻繁重設保留音高的解碼器造成聲音不連續；不會在解鎖成就時停播。

...
(async()=>{
 let {box,t,el}=boot();t.start();assert.equal(t.state,'running');t.jump();t.step(1/120);assert(t.player.y>0);const vy=t.player.vy;t.jump();assert.equal(t.player.vy,vy,'no double jump');
 t.pause();assert.equal(t.state,'paused');t.resume();assert.equal(t.state,'running');
 t.start();t.run.collectibles=[{x:t.player.x+8,y:30}];t.step(1/120);assert.equal(t.run.cards,1);assert(t.run.score>=100);
 for(let kind=0;kind<3;kind++){t.start();t.run.obstacles=[{x:t.player.x,w:64,h:kind===1?8:40,kind,rotation:0}];t.step(1/120);assert.equal(t.state,'over',`collision ${kind}`);await Promise.resolve();}
 t.start();t.run.distance=999.99;t.run.score=999.99;t.step(1/120);assert(t.run.arrived);assert.equal(t.state,'achievement');el('achievement').close();assert(t.run.score>=3000);assert(t.run.particles.length>0);assert.equal(t.run.obstacles.length,0);
 ...
 const reload=boot();assert.equal(reload.box.RunStore.getProfile().best,best);assert((await reload.box.RunStore.list('today')).some(r=>r.mine));reload.t.start();assert.equal(reload.t.run.score,0);
 ...
 t.start();t.step(1/120);const slow=t.run.speed-210;t.run.arrived=true;const beforeSpeed=t.run.speed;t.step(1/120);assert(Math.abs((t.run.speed-beforeSpeed)/slow-4)<1e-6);
 t.run.speed=10000;t.step(1/120);assert(t.run.speed>10000);
 t.start();t.run.nextCard=9;t.run.distance=8999.99;t.run.arrived=true;t.run.obstacles=[];t.step(1/120);assert.equal(t.state,'achievement');assert(t.run.achievement);assert.equal(el('achievement').open,true);assert.equal(t.run.obstacles.length,0);assert.equal(box.RunStore.getAchievement().distance,9000);
 ...
 t.start();assert.equal(t.run.achievement,undefined);
 // Each new planet has its own persistent collectible, including the starting planet.
 t.start();assert.equal(box.RunStore.getAchievement().planetIndex,0);
 ...
 t.start();t.run.speed=100000;t.run.obstacles=[{x:t.player.x+100,w:33,h:40,kind:0,rotation:0}];t.step(1/120);assert.equal(t.state,'over');
 ...
 const distances=[];for(const fps of [30,60,90,120]){const fresh=boot();fresh.t.start();fresh.t.run.safe=100;fresh.t.frame(0);for(let frame=1;frame<=fps*5;frame++)fresh.t.frame(frame*1000/fps);distances.push(fresh.t.run.distance);}assert(Math.max(...distances)-Math.min(...distances)<.5);
